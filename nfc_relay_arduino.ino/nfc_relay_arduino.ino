/*
 * Name： NFC control a relay
 * Author： John MCLear http://mclear.co.uk
 * Function： Read NFC and control Relay function
 */
...
#define PICC_AUTHENT1A        0x60               //verify A password keyf
...
void loop()
{
	...
	uchar status;
		...
		if (status == MI_OK)
		{
                        ...
                        id = String(serNum[0]) + String(serNum[1]) + String(serNum[2]) + String(serNum[3]) + String(serNum[4]);
                        Serial.print(id);
                        
                        // Should really check all pairs, but for now we'll just use the first
                     
                        if(id == "1364173173140" || id == "13644917316") {
                          pinMode(relay, INPUT);           // set pin to input
                          digitalWrite(relay, HIGH);       // turn on pullup resistors
                          Serial.println("Hello John");
                          delay(1000);
                          digitalWrite(relay, LOW);       // turn on pullup resistors
